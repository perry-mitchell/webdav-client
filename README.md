...
#### putFileContents(remotePath, format, data, overwriteFileIfExisting)
Put some data in a remote file at `remotePath` from a `Buffer` or `String`. `format` can be either "binary" or "text". `data` is a `Buffer` or a `String`. If `overwriteFileIfExisting` is set to false, server should abort put if `remotePath` is already existing (default is to overwrite).
