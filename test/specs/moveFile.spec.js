...
describe("moveFile", function() {
  ...
  it("moves files from one directory to another", function() {
    return this.client
      ...
      .then(function() {
        expect(
          fileExists(path.resolve(__dirname, "../testContents/alrighty.jpg"))
        ).to.be.false;
        expect(
          fileExists(
  ...
  it("moves directories from one directory to another", function() {
    return this.client.moveFile("/webdav", "/sub1/webdav").then(function() {
      expect(
        directoryExists(path.resolve(__dirname, "../testContents/webdav"))
      ).to.be.false;
      expect(
        directoryExists(path.resolve(__dirname, "../testContents/sub1/webdav"))
      ).to.be.true;
    });
  });
  ...
  it("moves files from one name to another", function() {
    return this.client
      ...
      .then(function() {
        ...
        expect(
          fileExists(path.resolve(__dirname, "../testContents/renamed.jpg"))
        ).to.be.true;
      });
  });
});
